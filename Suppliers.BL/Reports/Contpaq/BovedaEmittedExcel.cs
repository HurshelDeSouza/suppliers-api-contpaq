...
public class BovedaEmittedExcel
{
    public static async void Generate(BovedaEmittedData reportData, string fullPath)
    {
        if (reportData == null)
        {
            return;
        ...
        var sheet = book.CreateSheet("Boveda Emitted");
        ...
        // Agregar encabezado
        indexRow = sheet.AgregarEncabezado("Boveda Emitted", indexRow, reportData.Rows.Count);
        indexRow++;
        ...
        // Aplicar autofiltro
        sheet.SetAutoFilter(new NPOI.SS.Util.CellRangeAddress(rowTable, indexRow - 2, 0, headers.Length - 1));
